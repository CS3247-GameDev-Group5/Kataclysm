...
{	
	...
	GamepadInputMapping refMapping;
	public void setMapping(GamepadInputMapping mapping) {
		refMapping = mapping;
	}

	public override string GetButtonName(MappedButton button)
	{
		var mapping = refMapping.GetGamepadButtonMapping (button);
		if( mapping != null )
		{
	...
	{
		var mapping = refMapping.GetGamepadAxisMapping (axis);
		if( mapping != null )
		{
