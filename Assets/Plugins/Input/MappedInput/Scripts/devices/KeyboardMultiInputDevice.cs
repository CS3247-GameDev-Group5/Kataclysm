...
public class KeyboardMultiInputDevice : InputDevice
{	
	KeyboardInputMapping storedMapping;
	public void setMapping(KeyboardInputMapping mapping) {
		storedMapping = mapping;
		name = mapping.name;
	}
	public override string GetButtonName(MappedButton button)
	{
		var mapping = storedMapping.GetButtonMapping (button);
		if (mapping == null)
			return "";
	...
	{
		var mapping = storedMapping.GetAxisMapping (axis);
		if (mapping != null) {
